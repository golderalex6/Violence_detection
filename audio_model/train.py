...
class classify_audio():
    ...
    def evaluate(self):
        ...
        self.precision=precision_score(self.y,self.y_pred,average='micro')
        self.recall=recall_score(self.y,self.y_pred,average='micro')
        self.f1=f1_score(self.y,self.y_pred,average='micro')
        
        return pd.Series([self.accuracy,self.precision,self.recall,self.f1],index=['accuracy','precision','recall','f1'])

    def heat_map(self):
        self.confusion_matrix=confusion_matrix(self.y,self.y_pred)
        ...
        ax.grid(False)
        setting=[range(self.confusion_matrix.shape[0]),['Non violence','Vilolence']]
